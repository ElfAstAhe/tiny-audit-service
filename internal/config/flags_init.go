...
func initFLags() (res *pflag.FlagSet, err error) {
	...
	// Используем константы Flag...
	{
		...
		res.String(FlagLoginAttemptsReceiverKind, defaultLoginAttemptsReceiverKind, "login attempts receiver kind")
		// login attempts receiver worker
		res.Duration(FlagLoginAttemptsWorkerStartInterval, defaultLoginAttemptsWorkerStartInterval, "login attempts worker scheduler start interval")
		res.Duration(FlagLoginAttemptsWorkerScheduleInterval, defaultLoginAttemptsWorkerScheduleInterval, "login attempts worker scheduler schedule interval")
		res.Duration(FlagLoginAttemptsWorkerShutdownTimeout, defaultLoginAttemptsWorkerShutdownTimeout, "login attempts worker scheduler shutdown timeout")
		res.Int(FlagLoginAttemptsWorkerWorkerCount, defaultLoginAttemptsWorkerWorkerCount, "login attempts worker pool worker count")
		res.Int(FlagLoginAttemptsWorkerDataCapacity, defaultLoginAttemptsWorkerDataCapacity, "login attempts worker pool data capacity")
		res.Bool(FlagLoginAttemptsWorkerCompleteProcessing, defaultLoginAttemptsWorkerCompleteProcessing, "login attempts worker pool force complete processing")
		res.Int(FlagLoginAttemptsWorkerBatchSize, defaultLoginAttemptsWorkerBatchSize, "login attempts worker batch size")
		res.Duration(FlagLoginAttemptsWorkerBatchReadTimeout, defaultLoginAttemptsWorkerBatchReadTimeout, "login attempts worker batch read timeout")
		res.Duration(FlagLoginAttemptsWorkerAcknowledgeTimeout, defaultLoginAttemptsWorkerAcknowledgeTimeout, "login attempts worker receiver acknowledge timeout")
		// amqp login attempts receiver
		res.String(FlagLoginAttemptsReceiverAMQPConfigTargetName, defaultLoginAttemptsReceiverAMQPConfigTargetName, "amqp login attempts receiver queue/topic name")
		res.Duration(FlagLoginAttemptsReceiverAMQPConfigConnectTimeout, defaultLoginAttemptsReceiverAMQPConfigConnectTimeout, "amqp login attempts receiver connect timeout")
		res.Duration(FlagLoginAttemptsReceiverAMQPConfigShutdownTimeout, defaultLoginAttemptsReceiverAMQPConfigShutdownTimeout, "amqp login attempts receiver shutdown timeout")
		res.Int(FLagLoginAttemptsReceiverAMQPConfigPrefetchCredit, defaultLoginAttemptsReceiverAMQPConfigPrefetchCredit, "amqp login attempts receiver prefetch credit")
		...
		res.Int(FlagLoginAttemptsReceiverKafkaConfigMinBytes, conf.DefaultKafkaReceiverMinBytes, "kafka login attempts receiver min bytes")
		res.Int(FlagLoginAttemptsReceiverKafkaConfigMaxBytes, conf.DefaultKafkaReceiverMaxBytes, "kafka login attempts receiver max bytes")
		res.Duration(FlagLoginAttemptsReceiverKafkaConfigMaxWait, conf.DefaultKafkaReceiverMaxWait, "kafka login attempts receiver max wait")
		res.String(FlagLoginAttemptsReceiverKafkaConfigUsername, "", "kafka login attempts receiver username")
		res.String(FlagLoginAttemptsReceiverKafkaConfigPassword, "", "kafka login attempts receiver password")
		res.Duration(FlagLoginAttemptsReceiverKafkaConfigHeartbeatInterval, conf.DefaultKafkaReceiverHeartbeatInterval, "kafka login attempts receiver heartbeat interval")
		res.Duration(FlagLoginAttemptsReceiverKafkaConfigSessionTimeout, conf.DefaultKafkaReceiverSessionTimeout, "kafka login attempts receiver session timeout")
		res.Duration(FlagLoginAttemptsReceiverKafkaConfigRebalanceTimeout, conf.DefaultKafkaReceiverRebalanceTimeout, "kafka login attempts receiver rebalance timeout")
		res.Duration(FlagLoginAttemptsReceiverKafkaConfigReadTimeout, conf.DefaultKafkaReceiverReadTimeout, "kafka login attempts receiver read timeout")
		res.Int(FlagLoginAttemptsReceiverKafkaConfigMaxAttempts, conf.DefaultKafkaReceiverMaxAttempts, "kafka login attempts receiver max attempts")
		res.Int(FlagLoginAttemptsReceiverKafkaConfigQueueCapacity, conf.DefaultKafkaReceiverQueueCapacity, "kafka login attempts receiver queue capacity")
		res.String(FlagLoginAttemptsReceiverKafkaConfigStartOffset, conf.DefaultKafkaReceiverStartOffset, "kafka login attempts receiver start offset")
	}
